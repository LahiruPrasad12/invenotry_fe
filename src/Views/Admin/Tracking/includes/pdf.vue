...
export default {
  ...
  methods: {
    ...
    generatePDF() {
      ...
      doc.text(`Item Name : ${this.form.item.name}`, 85, 65)
      doc.text(`Item Code : ${this.form.item.item_code}`, 85, 72)

      // doc.text(`Printing Date : ${new Date()}`, 90, 20).setFontSize(8);
      doc.text(`Item Brand : ${this.form.item.brand}`, 85, 79)
      doc.text(`Supplier ID : ${this.form.item.supplier_id}`, 85, 86)
      doc.text(`Price : ${this.form.item.price}`, 85, 93)
      doc.text(`Item Quantity : ${this.form.item.qty}`, 85, 100)
      doc.text(`Status : ${this.form.orders.status}`, 85, 107)
